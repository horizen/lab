...
class lock_client_cache : public lock_client {
 private:
  ...
  std::string hostname;
  std::string id;
  std::map<lock_protocol::lockid_t, lockinfo*> lock_map_;
  std::vector<release_entry> release_queue_;
  pthread_mutex_t lock_map_m_;
  pthread_mutex_t release_queue_m_;
  pthread_cond_t release_queue_cond_;

  lock_protocol::status subscribe(std::string addr);

 public:
  static int last_port;
  lock_client_cache(std::string xdst, class lock_release_user *l = 0);
  virtual ~lock_client_cache() {
      pthread_mutex_destroy(&lock_map_m_);
      pthread_mutex_destroy(&release_queue_m_);
      pthread_cond_destroy(&release_queue_cond_);
  };
  lock_protocol::status acquire(lock_protocol::lockid_t);
  virtual lock_protocol::status release(lock_protocol::lockid_t);
  lock_protocol::status revoke(int, int, lock_protocol::lockid_t, int &);
  lock_protocol::status retry(int, lock_protocol::lockid_t, int &);
  void releaser();
};
#endif
