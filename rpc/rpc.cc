	...
	if (h.clt_nonce) {
		//have i seen this client before?
		{
			...
			if (reply_window_.find(h.clt_nonce) == reply_window_.end()) {
				assert (reply_window_[h.clt_nonce] == NULL); // create
				reply_window_[h.clt_nonce] = new std::list<reply_t>();
				jsl_log(JSL_DBG_2,
		...
		char *b, int sz)
{
    ...
    jsl_log(JSL_DBG_2, "unknown reply %u %u\n", clt_nonce, xid);
		...
		unsigned int xid_rep, char **b, int *sz)
{
	...
	if (reply_list->front().xid > xid) {
	    return FORGOTTEN;
	}

    while (reply_list->front().xid < xid_rep) {
        free(reply_list->front().buf);
        reply_list->pop_front();
    }

	if (reply_list->back().xid < xid) {
	    it = reply_list->end();
	    goto l_new_reply;
	}
	...
	for (it = reply_list->begin(); it != reply_list->end(); it++) {
	    if (xid == it->xid) {
	        jsl_log(JSL_DBG_2, "found duplicate request %u %u\n", clt_nonce, xid);
	        if (it->cb_present) {
	            return INPROGRESS;
	        }
	        *b = it->buf;
...
l_new_reply:
    ...
    reply_new.cb_present = true;
    reply_list->insert(it, reply_new);
...
operator>>(unmarshall &u, unsigned long &x)
{
    ...
    u >> h;
    u >> l;
    x = l | ((unsigned long) h << 32);
    return u;
}

unmarshall &
operator>>(unmarshall &u, long &x)
{
    unsigned int h, l;
    u >> h;
    u >> l;
